...
function generateAuthorizationPDF(
  ...
  // Encabezado con diseño mejorado (sin logo para mantener PDF liviano)

  // Nombre del club en azul
  doc.setFontSize(13);
  doc.setFont('helvetica', 'bold');
  doc.setTextColor(30, 100, 180); // Azul
  doc.text('CLUB AÉREO DE CASTRO', 105, 25, { align: 'center' });

  // Título principal en negro (dos líneas para mejor distribución)
  doc.setTextColor(0, 0, 0);
  doc.setFontSize(18);
  doc.setFont('helvetica', 'bold');
  doc.text('AUTORIZACIÓN DE VUELO', 105, 38, { align: 'center' });
  doc.text('PARA MENOR DE EDAD', 105, 47, { align: 'center' });

  // Doble línea decorativa en azul
  doc.setLineWidth(0.8);
  doc.setDrawColor(30, 100, 180); // Azul
        ...
        {step === 4 && (
          <div className="space-y-6">
            ...
            {flights.length === 0 ? (
              ...
              <div className="space-y-6">
                {/* Panel de pasajeros pendientes */}
                <div className="theme-bg-card rounded-2xl p-6 theme-shadow-md">
                  <h3 className="font-semibold theme-text-primary mb-3">
                    Pasajeros a asignar ({Object.keys(asignacionesIndividuales).length}/{cantidadPasajeros})
                  </h3>
                    ...
                    {pasajeros.map((pasajero, idx) => (
                      <div
                        ...
                        className={`px-3 py-2 rounded-lg text-sm font-medium transition-all ${
                          asignacionesIndividuales[idx]
                            ? 'bg-green-100 text-green-800 border border-green-300'
                            : 'bg-gray-100 theme-text-muted border theme-border'
                        }`}
                      >
                        {asignacionesIndividuales[idx] ? '✓ ' : '○ '}
                ...
                {/* Circuitos con selección de pasajeros */}
                {circuitosOrdenados.map((circuitoNum) => {
                  const vuelosCircuito = circuitos[circuitoNum];
                  const primerVuelo = vuelosCircuito[0];
                  const fechaVuelo = new Date(primerVuelo.fecha_hora);
                      ...
                      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
                        {vuelosCircuito.map((flight) => {
                          ...
                          const pasajerosSinAsignar = pasajeros
                            .map((_, idx) => idx)
                            .filter(idx => !asignacionesIndividuales[idx]);

                          return (
                            <div
                              key={flight._id}
                              className="p-5 rounded-xl border-2 theme-border transition-all"
                            >
                              <div className="flex items-center gap-3 mb-3">
                                <div className="text-3xl">✈️</div>
                                <div>
                              ...
                              <div className="space-y-2 mb-3">
                                <div className="flex justify-between">
                                  <span className="theme-text-muted text-sm">Disponibles</span>
                                  <span className={`font-bold ${cuposDisponibles > 0 ? 'text-green-600' : 'text-red-600'}`}>
                                    {cuposDisponibles}
                                  </span>
                                </div>
                              </div>

                              {/* Pasajeros asignados a este vuelo */}
                              {pasajerosAsignados.length > 0 && (
                                <div className="mb-3 p-2 bg-green-50 rounded-lg">
                                  <p className="text-xs font-semibold text-green-800 mb-1">
                                    Asignados:
                                  </p>
                                  {pasajerosAsignados.map((idx) => (
                                    <div key={idx} className="flex items-center justify-between text-xs py-1">
                                      <span className="text-green-700">
                                        • {pasajeros[idx].nombre} {pasajeros[idx].apellido}
                                      </span>
                                      <button
                                        type="button"
                                        onClick={() => {
                              ...
                              {/* Selector de pasajeros para asignar */}
                              {cuposDisponibles > 0 && pasajerosSinAsignar.length > 0 && (
                                <div className="space-y-2">
                                  <p className="text-xs theme-text-muted font-medium">
                                    Asignar pasajero:
                                  </p>
                                  <div className="space-y-1">
                                    {pasajerosSinAsignar.map((idx) => (
                                      <button
                                        key={idx}
                                        type="button"
                                        onClick={() => {
                                        ...
                                        className="w-full px-3 py-2 text-left text-xs rounded-lg bg-blue-50 hover:bg-blue-100 theme-text-primary border border-blue-200 hover:border-blue-400 transition-all"
                                      >
                                        + {pasajeros[idx].nombre} {pasajeros[idx].apellido}
                                        {pasajeros[idx].esMenor && ' 👶'}
                                      </button>
                                    ))}
                                  </div>
                                </div>
                              ...
                              {cuposDisponibles === 0 && pasajerosAsignados.length === 0 && (
